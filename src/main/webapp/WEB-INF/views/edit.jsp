			...
			<div class="row">
				<div class="col-8">
					<table>
						<tr>
							<td>Book Code</td>
							<td id="bookCode"><c:out value="${book.getBookCode() }"></c:out></td>
						</tr>
						<tr>
							<td>Book Name</td>
							<td><input type="text" name="bookName" id="bookName"
								value="${book.getBookName() }" required="required"></td>
						</tr>
						<tr>
							<td>Author</td>
							<td><select id="bookAuthor">
									<c:forEach var="author" items="${authorList}">
										...
										<c:if
											test="${author.getAuthorId()==book.getAuthor().getAuthorId() }">

											<option value="${author.getAuthorId()}" selected="selected"><c:out
													value="${author.getAuthorName()}"></c:out></option>
										</c:if>
										<c:if test="${author.getAuthorId()!=book.getAuthor().getAuthorId() }">
						...
						<tr>
							<td>Added On</td>
							<td id="dateAdded"><c:out value="${book.getDateAdded() }"></c:out></td>
						</tr>

					</table>
					<div class="mt-4">
						<span class="btn btn-primary buttons" id="editBook">Submit</span> <a href="home"
